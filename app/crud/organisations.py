...
def add_member_to_organisation(session: Session, user_id: int, organisation_id: int, role: UserRole) -> UserOrganisationLink:
    try:
        ...
        if existing_link:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="User is already a member of this organisation")

        user_org_link = UserOrganisationLink(user_id=user_id, organisation_id=organisation_id, role=role)
...
def remove_member_from_organisation(session: Session, user_id: int, organisation_id: int) -> None:
    try:
        ...
        if not user_org_link:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="UserOrganisationLink not found")

        if user_org_link.role == UserRole.OWNER:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Cannot remove the owner of the organisation")
