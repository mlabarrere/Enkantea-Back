...
def create_organisation(organisation_create: OrganisationCreate) -> OrganisationRead:
    """
...
def update_organisation(organisation_id: int, organisation_update: OrganisationUpdate) -> OrganisationRead:
    """
        ...
        try:
            ...
            if not db_organisation:
                raise OrganisationNotFoundError(
                    f"Organisation with id {organisation_id} not found"
                )
...
def get_organisation_by_id(organisation_id: int) -> OrganisationRead:
    """
...
def delete_organisation(organisation_id: int) -> OrganisationRead:
    """
...
def get_members_from_organisation(organisation_id: int) -> list[UserRead]:
    """
    ...
    with get_db_session() as session:
        ...
        statement = (
            ...
            .where(UserOrganisationLink.organisation_id == organisation_id)
        )
        results = session.exec(statement).all()
        return [UserRead.model_validate(user) for user, _ in results]


def add_member_to_organisation(user_id: int, organisation_id: int, role: UserRole) -> UserOrganisationLink:
    """
...
def remove_member_from_organisation(user_id: int, organisation_id: int) -> None:
    """
