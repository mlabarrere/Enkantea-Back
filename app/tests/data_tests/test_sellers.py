...
from app.data_layer.sellers import create_seller, get_seller_by_id, update_seller, delete_seller, get_sellers, get_sellers_by_organisation
...
def test_update_seller(db_session):
    ...
    update_data = SellerUpdate(first_name="Jane", email="jane.doe@example.com", professional=True)
    result = update_seller(session=db_session, seller_id=created_seller.id, seller_update=update_data)
...
def test_get_sellers_by_organisation(db_session):
    ...
    results_org1 = get_sellers_by_organisation(session=db_session, organisation_id=org1.id)
    results_org2 = get_sellers_by_organisation(session=db_session, organisation_id=org2.id)
