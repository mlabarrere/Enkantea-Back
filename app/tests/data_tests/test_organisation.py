import pytest
from app.models.organisations import OrganisationCreate, OrganisationRead, OrganisationUpdate, CompanyType
...
def test_get_organisation_by_id(db_session):
    ...
    created_org = create_organisation(session=db_session, organisation_create= org_create)
...
def test_update_organisation(db_session):
    ...
    update_data = OrganisationUpdate(name="Updated Organisation", organisation_type=CompanyType.SARL)
    result = update_organisation(session=db_session, organisation_id=created_org.id, organisation_update=update_data)
    ...
    partial_result = update_organisation(session=db_session, organisation_id=created_org.id, organisation_update=partial_update)
    assert partial_result.name == "Updated Organisation"
    assert partial_result.siren_number == 987654321

    with pytest.raises(OrganisationNotFoundError):
        update_organisation(session=db_session, organisation_id=9999, organisation_update=OrganisationUpdate(name="Nonexistent"))
