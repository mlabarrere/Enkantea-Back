...
from app.models.organisations import OrganisationCreate, OrganisationUpdate, CompanyType, Organisation
...
def test_create_organisation(session: Session) -> None:
    ...
    organisation = create_organisation(session=session, organisation_create=organisation_in)
...
def test_update_organisation(session: Session) -> None:
    ...
    updated_organisation = update_organisation(session=session, db_organisation=organisation, organisation_in=update_in)
...
def test_get_organisation_by_id(session: Session) -> None:
    ...
    organisation_in = OrganisationCreate(**orga_data_tests)
    organisation = create_organisation(session=session, organisation_create=organisation_in)
    fetched_organisation = get_organisation_by_id(session=session, organisation_id=organisation.id)
...
def test_get_organisation_by_name(session: Session) -> None:
    ...
    organisation_in = OrganisationCreate(**orga_data_tests)
    organisation = create_organisation(session=session, organisation_create=organisation_in)
    fetched_organisation = get_organisation_by_name(session=session, company_name=organisation.company_name)
    assert fetched_organisation
...
def test_delete_organisation(session: Session) -> None:
    ...
    organisation = create_organisation(session=session, organisation_create=organisation_in)
    deleted_organisation = delete_organisation(session=session, organisation_id=organisation.id)
...
def test_remove_member_from_organisation(session: Session) -> None:
    ...
    remove_member_from_organisation(session=session, user_id=user.id, organisation_id=organisation.id)
    ...
    owner_link = UserOrganisationLink(user_id=user.id, organisation_id=organisation.id, role=UserRole.OWNER)
